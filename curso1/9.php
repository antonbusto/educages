...
$sql = "SELECT * 
		FROM curso" . $_SESSION['curso'] . "_paxinas WHERE posicion=1
		ORDER BY curso" . $_SESSION['curso'] . "_paxinas.nome";
$resultado = mysqli_query($conexion, $sql ) or die(mysqli_error($conexion));
echo "<div style='padding-left:30px;'>
	 <ul class='list-group'>";
	 while($fila = mysqli_fetch_assoc($resultado))
     {
	 echo '<li class="list-group-item"><a href="' . $fila['paxina'] . '.php" title="' . ucfirst($fila['nome']) . '"><strong>' . ucwords($fila['nome']) . '</strong></a><br />' . nl2br($fila['axuda']) . '</li><br /><br />';    
	 }
echo '</ul></div>';
