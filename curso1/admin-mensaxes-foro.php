...
$nome="administracion de mensajes del foro";
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
echo "<p class='text-center'>No tienes el permiso de acceso necesario</p>";
	...
	echo "<h4 class='text-center'><strong>Administraci&oacute;n de Mensajes del foro</strong></h4>";

//se arma la consulta para los usuarios del curso
$sql="SELECT * 
	  FROM curso" . $_SESSION['curso'] . "_foro_mensaxes";
...
echo "<table class='table table-striped'>
	   ...
	   usando el bucle do para presentar el primer registro */
	    ...
	    echo "
		<tr>
		<td>" . $fila['id_mensaxe'] . "</td>
		<td valign='top'>" . htmlentities($fila['contido_mensaxe']) . "</td>
		<td valign='top'>" . $fila['data_mensaxe'] . "</td>
		<td valign='top'>" . $fila['tema_mensaxe'] . "</td>
		<td valign='top'>" . $fila['por_mensaxe'] . "</td>
		<td valign='top'><a href='modificar-mensaxe-foro.php?id=" . $fila['id_mensaxe'] . "' title='Modificar' class='btn btn-success'>Modificar</a></td>
		<td valign='top'><a href='eliminar-mensaxe-foro.php?id=" . $fila['id_mensaxe'] . "' title='Borrar' class='btn btn-danger'>Borrar</a></td>
		</tr>";
...
echo"
	<tr><td colspan='6' class='dereita'><strong>Total mensajes:</strong></td><td><span class='badge badge-secondary'>" . $total_filas . "</span></td></tr>
	</table> </div>";
