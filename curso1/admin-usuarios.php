...
echo "<h4 class='text-center'><strong>Administraci&oacute;n de usuarios</strong></h4>";

//se aclara el significado del algunos campos
echo "<p class='small'>Nivel 0: usuario pendiente de aprobaci&oacute;n <br /> Nivel 1: usuario aprobado para el curso que no tiene permisos de administrador <br /> Nivel 2: usuario con permisos de administrador</p>";


//se arma la consulta para los usuarios del curso
$sql="SELECT * 
	  FROM usuarios 
	  WHERE curso =" . $_SESSION['curso'] . "";
...
echo "<p><a href='crear-usuarios-curso.php' title='Crear usuarios' class='btn btn-success'>Crear nuevo usuario</a></p>";
...
<table class='table table-striped'>  
	   ...
	   usando el bucle do para presentar el primer registro */
		...
		<td>" . $fila['contrasinal_usuario'] . "</td>
		<td>" . $fila['email_usuario'] . "</td>
		<td>" . $fila['data_usuario'] . "</td>
		<td>" . $fila['telefono_usuario'] . "</td>
		<td>" . $fila['curso'] . "</td>
		<td>" . $fila['nivel_usuario'] . "</td>
		<td><a href='modificar-usuario.php?id=" . $fila['id_usuario'] . "' title='Modificar' class='btn btn-success'>Modificar</a></td>
		<td><a href='eliminar-usuario.php?id=" . $fila['id_usuario'] . "' title='Borrar' class='btn btn-danger'>Borrar</a></td>
		</tr>";
...
echo"
	<tr><td colspan='9'><strong>Total usuarios:</strong></td><td><span class='badge badge-secondary'>" . $total_filas . "</span></td></tr>
	</table> </div>";
